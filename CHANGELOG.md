...
### v1.7.3 (2026-02-09)
- 🔐 **认证提示与自检**：新增页面提示与设置内测试按钮，支持匿名检测服务器/API Key，并在认证失效时提醒用户
- ⚠️ **错误阻断优化**：401 统一识别为认证错误，必要时停止批量检索，避免无效请求

### v1.7.2 (2026-02-09)
- 🔐 **修复未登录时认证失败** - API Key 现在以 Base64 Bearer 头并附带 apikey 参数发送，无需先登录 LRR 也能查询
- 🖼️ **缩略图拉取对齐认证** - 缩略图请求同样附带 API Key，避免返回空/错误数据导致不显示
...
| 脚本 | 当前版本 | 发布日期 | 状态 |
|------|--------|--------|------|
| EhSearchEnhancer | v2.3.6 | 2026-03-05 | ✅ 稳定 |
| AriaEh | v1.3.1 | 2026-01-20 | ✅ 稳定 |
| ExHentai Lanraragi Checker | v1.7.3 | 2026-02-09 | ✅ 稳定 |
